...
feature_extractor.add(Dense(1, activation = 'sigmoid'))
...
history = feature_extractor.fit(x_train, y_train, batch_size = 50, epochs=20, validation_data = (x_test, y_test))
...
prediction_treatment = feature_extractor.predict(x_test)

predictions = []
for item in range(len(prediction_treatment)):
    if prediction_treatment[item] < 0.5:
        predictions.append(0)
    elif prediction_treatment[item]>0.5:
        predictions.append(1)

prediction_NN = np.argmax(prediction_treatment, axis=-1)
